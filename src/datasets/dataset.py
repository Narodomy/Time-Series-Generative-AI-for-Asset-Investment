...
class MarketDataset(Dataset):
    def __init__(
        ...
        windows_x = sliding_window_view(self.x, window_shape=window_size, axis=0)[::self.stride] # [Num_Window, Assets, Features, Window_Size]
        windows_cond = sliding_window_view(self.cond, window_shape=window_size, axis=0)[::self.stride] # [Num_Window, Assets, Features_cond, Window_Size]
        windows_date = sliding_window_view(self.date, window_shape=window_size, axis=0)[::self.stride] # [Num_Window, Window_Size]
        windows_price = sliding_window_view(self.price, window_shape=window_size, axis=0)[::self.stride] # [Num_Window, Assets, Feature price, Window_Size]
        ...
        self.windows_x = windows_x.transpose(0, 2, 3, 1) # [Num_windows, Features, Window_size, Assets]
        self.windows_cond = windows_cond.transpose(0, 2, 3, 1) # [Num_windows, Features, Window_size, Assets]
        self.windows_date =  windows_date
        self.windows_price = windows_price.transpose(0, 2, 3, 1) # [Num_Windows, Features, Window_Size, Assets]
