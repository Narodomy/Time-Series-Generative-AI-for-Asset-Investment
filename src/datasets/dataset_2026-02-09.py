...
class MarketDataset(Dataset):
    def __init__(self, data: np.ndarray, prices: np.ndarray, dates: np.ndarray, window_size: int, target_idx: int = 3):
        # [T, A, F]
        ...
        self.date_windows = sliding_window_view(self.dates, window_shape=window_size, axis=0)
        self.price_windows = sliding_window_view(self.prices, window_shape=window_size, axis=0)
    ...
    def __getitem__(self, idx: int):
        ...
        window_dates = self.date_windows[idx].copy().astype(np.int64)
        window_prices = self.price_windows[idx].copy().astype(np.float64)
        # x shape: [T, A, 1] 
