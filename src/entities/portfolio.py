...
class PortfolioResult:
    ...
    window_returns: list[pd.Series]  # ← เพิ่ม
    window_portfolios: list[vbt.Portfolio]
...
class Portfolio:
    ...
    def convert_to_portfolio(
        ...
        if is_log_return:
            price = init_cash * np.exp(returns.cumsum())
        else:
            price = init_cash * (1 + returns).cumprod()
    ...
    def evaluate(
        ...
        mc_returns: Optional[
            Annotated[np.ndarray, ("n_paths", "n_lengths", "n_assets")]
        ],
        rebalance_freq: int,
        optimize_window: int,
        start_date: Optional[date] = None,
        ...
        if start_date is not None and end_date is not None:
            dates = pd.bdate_range(start=start_date, periods=n_periods)
        else:
            dates = pd.bdate_range(start="2000-01-01", periods=n_periods)

        # equal-weight ถ้า mc_returns=None
        is_equal_weight = mc_returns is None
        if is_equal_weight:
            fixed_weights = np.ones(n_assets) / n_assets
            logger.info("evaluate: using equal-weight (mc_returns=None)")
        ...
        window_portfolios_list: list[vbt.Portfolio] = []
        ...
        for t_start, t_end, optimize_start, optimize_end in windows:
            ...
            window_returns_list.append(series)
            window_portfolios_list.append(
                self.convert_to_portfolio(
                    returns=series, init_cash=init_cash, is_log_return=is_log_return
                )
            )
        ...
        return PortfolioResult(
            ...
            window_returns=window_returns_list,
            window_portfolios=window_portfolios_list,
        )
    ...
    def experiment(
        ...
        # --- 1. เช็คเงื่อนไข Window Size = n_lengths (skip ถ้า equal-weight) ---
        for name, mc_returns in strategies.items():
            if mc_returns is None:
                continue  # equal-weight ไม่ต้องเช็ค
