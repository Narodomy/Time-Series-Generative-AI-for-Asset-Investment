...
class Portfolio:
    ...
    def back_test(
        ...
        sim_paths: Optional[List[np.ndarray]] = None,
        ...
        Weights can be supplied in two ways — exactly one must be provided:
          - weights   : (N,) pre-computed weights (skip internal optimization).
          - sim_paths : List of (Steps, N) simulated return paths; weights are
                        derived internally via mu_sigma_method + aggregate_method.

        Args:
            returns               : (T, N) actual (historical) asset returns for NAV simulation.
            weights               : (N,) pre-computed portfolio weights. Mutually exclusive with sim_paths.
            sim_paths             : List of (Steps, N) simulated paths for internal optimization.
            ...
            benchmark_tc_rate     : TC for benchmark. Defaults to same as transaction_cost_rate.
            risk_free_rate        : Override instance rf for optimization.
            solver                : "scipy" or "pypfopt". Used when sim_paths provided.
            mu_sigma_method       : "combined" → pool all sim paths into one mu/sigma.
                                    "separate" → optimize per-path then aggregate.
            aggregate_method      : Used when mu_sigma_method="separate".
                                    "mean" | "median" | "top_k_sharpe"
            top_k                 : Top-k sims to use when aggregate_method="top_k_sharpe".
            ...
            filename              : Report filename (no extension).
                                    Defaults to "Portfolio_Report_<today>".
        Returns:
            metrics : pd.DataFrame with full QuantStats metric table.
        """
        # Validate input — exactly one of weights / sim_paths must be given
        if weights is None and sim_paths is None:
            raise ValueError("Provide either 'weights' or 'sim_paths'.")
        if weights is not None and sim_paths is not None:
            raise ValueError("Provide either 'weights' or 'sim_paths', not both.")

        rf = risk_free_rate if risk_free_rate is not None else self.risk_free_rate

        # Derive weights from sim_paths if not pre-computed
        if sim_paths is not None:
            if mu_sigma_method == "combined":
                mu, sigma = self.calc_distribution(sim_paths)
                weights = self.optimize_weights(mu, sigma, rf, solver)
            elif mu_sigma_method == "separate":
                weights = self.optimize_weights_separate(
                    sim_paths, rf, solver, aggregate_method, top_k
                )
            else:
                raise ValueError(
                    f"Unknown mu_sigma_method '{mu_sigma_method}'. "
                    "Choose 'combined' or 'separate'."
                )
            logger.info(
                f"Weights derived from sim_paths | method={mu_sigma_method} "
                f"| top asset={np.argmax(weights)} | max_w={np.max(weights):.3f}"
            )
