...
class Portfolio:
    ...
    def convert_to_portfolio(
        self, returns: pd.Series, init_cash: float = 1.0, is_log_return: bool = True
    ) -> vbt.Portfolio:
        """
        แปลง Series ของ Returns ให้กลายเป็น object vbt.Portfolio ของ vectorbt
        """
        from pandas.tseries.offsets import BDay  # สำหรับดึงวันทำการ

        # 1. หากเป็น Log Return ให้แปลงกลับเป็น Simple Return
        if is_log_return:
            simple_returns = inverse_log_returns(returns)
        else:
            simple_returns = returns

        # 2. จำลองเส้นราคา (Cumulative Price) โดยเริ่มสะสมตั้งแต่วันแรก
        cum_price = (1 + simple_returns).cumprod() * init_cash

        # 3. สร้าง Day 0 เพื่อไม่ให้ผลตอบแทนของวันแรกสุดตกหล่น
        # ถอยกลับ 1 วันทำการ (Business Day) จากวันแรกสุดของข้อมูล
        start_date = simple_returns.index[0] - BDay(1)

        # นำ Day 0 (ราคา = init_cash) มาต่อเชื่อมกับเส้นราคาด้านหน้า
        price_series = pd.concat(
            [pd.Series([init_cash], index=[start_date]), cum_price]
        )

        # 4. สร้างพอร์ตโฟลิโอผ่าน from_holding
        # (จำลองการเข้าซื้อด้วยเงินทั้งหมดใน Day 0 แล้วถือยาว ทำให้ได้ผลตอบแทนของ Day 1 ครบถ้วน)
        portfolio = vbt.Portfolio.from_holding(
            price_series, init_cash=init_cash, freq="B"  # B = Business days
        )
    ...
    def experiment(
        ...
        # --- [จุดแก้ไขที่ 1] เช็คเงื่อนไขพารามิเตอร์เทียบกับ window size (n_lengths) ---
        if strategies:
            # ดึงขนาดของ n_lengths จาก strategy แรกใน dict มาเป็น window_size
            first_strategy_matrix = next(iter(strategies.values()))
            window_size = first_strategy_matrix.shape[1]  # มิติตรงกลางคือ n_lengths

            if optimize_window >= window_size:
                raise ValueError(
                    f"เงื่อนไขไม่ถูกต้อง: optimize_window ({optimize_window}) ต้อง น้อยกว่า window_size ({window_size}) เสมอ!"
                )
            if rebalance_freq >= window_size:
                raise ValueError(
                    f"เงื่อนไขไม่ถูกต้อง: rebalance_freq ({rebalance_freq}) ต้อง น้อยกว่า window_size ({window_size}) เสมอ!"
                )
        # -----------------------------------------------------------------------
        ...
        # สร้าง Rebalance Windows ล่วงหน้าเพื่อนำไปใช้หั่นสไลด์ QuantStats และใช้วาดกราฟด้านล่าง
        n_periods = gt_returns.shape[0]
        rebalance_windows = self.build_rebalance_window(
            n_periods=n_periods,
            rebalance_freq=rebalance_freq,
            optimize_window=optimize_window,
        )

        # --- [จุดแก้ไขที่ 2] แยกการจัดพอร์ตและทำรายงาน QuantStats เป็นราย Window ---
        for name, portfolio in portfolios.items():
            ...
            # วนลูปเพื่อหั่นสไลด์ข้อมูลทำ QuantStats ตามช่วงเวลาของแต่ละ Window
            for idx, (t_start, t_end, _, _) in enumerate(rebalance_windows):
                window_strat_returns = strategy_returns.iloc[t_start:t_end]
                window_bench_returns = benchmark_returns.iloc[t_start:t_end]

                title = f"{name} vs {benchmark_strategy} - Window {idx+1} ({method}, Rebal: {rebalance_freq}, Window: {optimize_window})"

                if output_dir is not None:
                    os.makedirs(output_dir, exist_ok=True)
                    # ตั้งชื่อไฟล์ html แยกตามเลข Window เช่น strategy_vs_benchmark_window_1.html
                    output_path = os.path.join(
                        output_dir,
                        f"{name}_vs_{benchmark_strategy}_window_{idx+1}{output_suffix or ''}.html",
                    )
                    qs.reports.html(
                    ...
                    logger.info(
                        "Report for Window %d saved to: %s", idx + 1, output_path
                    )
                else:
                    qs.reports.full(
                        window_strat_returns,
        ...
        if gt_prices is not None:
            ...
            self.plot_trades_all_assets(
                gt_prices=gt_prices,
                results=results,
                rebalance_windows=rebalance_windows,  # ใช้ตัวแปรที่ประกาศไว้ด้านบนร่วมกันได้เลย
                dates=dates,
                init_cash=init_cash,
