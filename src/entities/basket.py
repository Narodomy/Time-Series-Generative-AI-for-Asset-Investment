...
# from junk.src.strategies import AlignmentStrategy
...
class Basket:
    def __init__(self, symbols: List[str], device: torch.device = torch.device("cuda")):
        ...
        logger.debug(
            f"Initialized Asset Basket: {self.symbols} with {len(self.assets)} assets which loaded."
        )
    ...
    def get_unique_features(self) -> List[str]:
        ...
        unique_cols = {
            col for asset in self.assets.values() for col in asset.data.columns
        }
    ...
    def load_all_assets(self, freq="1d"):
        ...
        logger.info(
            f"Batch load complete. Success: {success_count}/{len(self.symbols)}. Total assets in basket: {len(self.assets)}"
        )
    ...
    def align(self, strategy: AlignmentStrategy, inplace: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            if inplace:
                ...
                for symbol, asset in self.assets.items():
                    asset.data = asset.data.loc[
                        asset.data.index.intersection(common_index)
                    ]

                logger.info(
                    f"Assets updated in-place to aligned index (Length: {len(common_index)})"
                )
    ...
    def plot_assets(self, column, title="Basket Composition"):
        ...
        if plotted_count > 0:
            ...
            logger.warning(
                f"No assets found with column '{column}'. Did you calculate returns?"
            )
