...
class ExperimentManager:
    ...
    def save_training_results(self, engine):
        ...
        # val_series = pd.Series(history["val_loss"])
        # valid_mask = val_series.notna()

        # if valid_mask.any():
        #     best_idx = val_series[valid_mask].idxmin()  # index จริงใน original series
        #     best_epoch = int(best_idx) + 1
        #     best_val = float(val_series[best_idx])
        # else:
        #     # ไม่มี val_loss ดีเลย — ข้าม axvline แต่ plot ได้ต่อ
        #     best_epoch = None
        #     best_val = float("nan")
        ax.axvline(
            best_epoch,
        ...
        report = {
            ...
            "config": {
                ...
                "noise_steps": self.config.model.noise_steps,
    ...
    def save_evaluation_results(
        ...
        summary = {
            ...
            "eval_config": {
                "annual_risk_free_rate": self.config.annual_risk_free_rate,  # BUG FIX: was 'risk_free_rate'
                "rebalance_frequency": self.config.rebalance_frequency,  # BUG FIX: was 'rebalance_days'
