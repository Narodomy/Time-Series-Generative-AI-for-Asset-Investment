...
class FeatureConfig:
    ...
    test_start_date: str = "2022-01-03"
    ...
    indicators: List[Indicator] = field(
        default_factory=lambda: [
            ...
            # ตัดออก
            # SMA_20 → แทนด้วย ATR ดีกว่า บอก volatility context ได้จริง
            # เพิ่มใหม่
            Indicator(name="ATR", params={"timeperiod": 14}, norm_type="minmax"),
            # ^ วัด realized volatility — สำคัญมากตอน 2022 bear market
            Indicator(name="ROC", params={"timeperiod": 10}, norm_type="minmax"),
            # ^ short-term momentum — ช่วยให้ model รู้ direction ของ price movement
        ]
    )
...
class TrainConfig:
    ...
    # lr: float = 1e-4
    # lr: float = 1e-5
    lr: float = 0.0007343610150369279
    # weight_decay: float = 1e-6
    weight_decay: float = 2.282803889803081e-06
...
class ModelConfig:
    ...
    noise_steps: int = 1750
    beta_start: float = 0.00022330845259098812
    beta_end: float = 0.04816694985809928
    ...
    max_t: int = 128 * 2
    dropout: float = 0.1
    dim_feedforward: int = 512 * 2  # ffmultiplier = 2
...
class Config:
    ...
    description: str = (
        "DDPM Transformer inside laten space from [B, A, T, C] when compute [B, A, T*C]"
    )
