...
class Engine:
    ...
    def _reshape(
        self,
        x: torch.Tensor,  # (B, A, W, D, C)
        cond: torch.Tensor,  # (B, A, W, F)
    ) -> Tuple[torch.Tensor, torch.Tensor]:

        return x, cond

    def _predict(self, x: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        x, cond = self._reshape(x, cond)  # (B, seq, feat)
    ...
    def fit(
        ...
        for epoch in range(1, epochs + 1):
            ...
            if val_loss is not None and is_save_best and val_loss < best_val_loss:
                ...
                self.save_checkpoint("best_model.pt", epoch=epoch, val_loss=val_loss)
                logger.info(f"  ↓ best model saved  (val={val_loss:.4f})")

            # Periodic checkpoint
            if save_every > 0 and epoch % save_every == 0:
                self.save_checkpoint(
                    f"epoch_{epoch:04d}.pt", epoch=epoch, val_loss=val_loss
                )
    ...
    def save_checkpoint(
        self, filename: str, epoch: int = None, val_loss: float = None
    ) -> None:
    ...
    def _plot_loss_curve(self) -> None:
        ...
        if self.history["val_loss"]:
            ...
            min_ep = self.history["val_loss"].index(min_val) + 1
            ax.annotate(
                f"best: {min_val:.4f}\nep {min_ep}",
                xy=(min_ep, min_val),
                xytext=(min_ep + len(self.history["val_loss"]) * 0.03, min_val + 0.1),
                arrowprops=dict(arrowstyle="->", color="gray"),
                fontsize=8,
                color="darkorange",
            )

            # annotate spikes (val loss > mean + 3*std)
            val_arr = np.array(self.history["val_loss"])
            threshold = val_arr.mean() + 3 * val_arr.std()
            for ep, v in zip(epochs, val_arr):
                if v > threshold:
                    ax.annotate(
                        f"{v:.2f}",
                        xy=(ep, v),
