...
class DiffusionTransformer(nn.Module):
    ...
    # ── Dim handling ────────────────────────────────────────────
    # Centralize the (W, A) <-> S reshape logic here so forward()
    # can stay 4D in/4D out. Adjust these two methods if the
    # flatten strategy changes (e.g. interleave vs concat axes,
    # separate W/A positional encodings, etc.) — nothing else
    # in the model needs to change.
    def _flatten(self, t: torch.Tensor) -> torch.Tensor:
        # (B, W, A, C) → (B, S=W*A, C)
        B, W, A, C = t.shape
        return t.reshape(B, W * A, C)

    def _unflatten(self, t: torch.Tensor, W: int, A: int) -> torch.Tensor:
        # (B, S=W*A, C) → (B, W, A, C)
        B, S, C = t.shape
        return t.reshape(B, W, A, C)
    ...
    def forward(
        ...
        x_flat = self._flatten(x)  # (B, S, C_x)
        cond_flat = self._flatten(cond)  # (B, S, C_c)
        S = x_flat.shape[1]

        h_x = self.x_proj(x_flat)  # (B, S, d_model)
        h_cond = self.cond_proj(cond_flat)  # (B, S, d_model)
        ...
        out_flat = self.output_proj(out_h)  # (B, S, feat_x)

        out = self._unflatten(out_flat, W, A)  # (B, W, A, C_x)
