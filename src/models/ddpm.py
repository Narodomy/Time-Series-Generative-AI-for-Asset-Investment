...
class Diffusion(nn.Module):
    ...
    def sample(self, x_cond: Tensor, output_shape: Size) -> Tensor:
        ...
        Standard generation.
        Args:
            x_cond      : (B, seq, feat_cond) — already reshaped by Engine
            output_shape: (B, seq, feat_x)    — same layout as x_cond
        """
    ...
    def create_lower_right_triangle_mask(self, x: torch.Tensor):
        ...
        base_mask = self.process_lower_right_triangle(T, C).to(device)  # [T, C]

        # [T, C] → [1, 1, T, C] → broadcast [B, A, T, C]
        final_mask = base_mask.reshape(1, 1, T, C).expand(B, A, -1, -1)
