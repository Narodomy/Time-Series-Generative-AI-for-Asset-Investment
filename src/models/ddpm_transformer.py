...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        # Feature
        self.input_dim_x = num_assets * num_channels # A*(Channels_target + Channels_condition)
        print(f"input dim x: {self.input_dim_x}, d_model: {d_model}")
        self.x_proj = nn.Linear(self.input_dim_x, d_model) # Projection [Batch, Seq_length, d_model]

        # Condition
        self.input_dim_cond = num_assets * num_cond_channels
        self.cond_proj = nn.Linear(self.input_dim_cond, d_model) # Projection [Batch, Seq_length, d_model]
        ...
        self.output_dim = num_assets * num_channels # we just don't need condition include from prediction!
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        # Embedding
        t_emb = self.time_mlp(t) # [B, d_model] put fourier transform to say that have noise at t step time! (normaly, it's linear and adjust littke non-linear with GeLU but not much to say that have much noise!)
        ...
        out = out.reshape(B, L, A, self.num_channels).permute(0, 3, 1, 2) # [B, C, L, A]
